...
int main()
{
    ...
    while(1){
        ...
        {
            if(0 != pthread_create(&thread1,NULL,thread_routine,&recibido)){
                return -1;
            }
...
{   
    ...
    int recibido=*((int*)arg);
    if(bandera!=recibido){
    cargarMatriz("asientos.txt",matriz);
    reservarAsiento(recibido,matriz);
    guardarMatriz(matriz);
    printf("Asiento %i reservado\n", recibido);
    bandera=recibido;
    }
